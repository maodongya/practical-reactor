...
/**
 * In this chapter we will learn difference between hot and cold publishers, how to split a
 * publisher into multiple and how to keep history so late subscriber don't miss any updates.
 *
 * <p>Read first:
 *
 * <p>https://projectreactor.io/docs/core/release/reference/#reactor.hotCold
...
public class c12_Broadcasting extends BroadcastingBase {
  ...
  /**
   * Split incoming message stream into two streams, one contain user that sent message and second
   * that contains message payload.
   */
  @Test
  public void sharing_is_caring() throws InterruptedException {
    Flux<Message> messages = messageStream().publish().refCount(2);
    ...
    Assertions.assertEquals(
        Arrays.asList("user#0", "user#1", "user#2", "user#3", "user#4"), metaData);
    Assertions.assertEquals(
        Arrays.asList("payload#0", "payload#1", "payload#2", "payload#3", "payload#4"), payload);
  }

  /**
   * Since two subscribers are interested in the updates, which are coming from same source, convert
   * `updates` stream to from cold to hot source. Answer: What is the difference between hot and
   * cold publisher? Why does won't .share() work in this case?
   */
  @Test
   ...
   * Adapt previous solution so second subscriber will get all updates, even the one's that were
   * broadcaster before its subscription.
   */
  @Test
