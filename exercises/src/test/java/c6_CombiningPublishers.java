 ...
 * <p>Read first:
 *
 * <p>https://projectreactor.io/docs/core/release/reference/#which.values
 *
 * <p>Useful documentation:
 *
 * <p>https://projectreactor.io/docs/core/release/reference/#which-operator
...
public class c6_CombiningPublishers extends CombiningPublishersBase {
  ...
  /**
   * Goal of this exercise is to retrieve e-mail of currently logged-in user. `getCurrentUser()`
   * method retrieves currently logged-in user and `getUserEmail()` will return e-mail for given
   * user.
   *
   * <p>No blocking operators, no subscribe operator! You may only use `flatMap()` operator.
   */
  @Test
  ...
  /**
   * `taskExecutor()` returns tasks that should execute important work. Get all the tasks and
   * execute them.
   *
   * <p>Answer: - Is there a difference between Mono.flatMap() and Flux.flatMap()?
   */
  @Test
  ...
  /**
   * `streamingService()` opens a connection to the data provider. Once connection is established
   * you will be able to collect messages from stream.
   *
   * <p>Establish connection and get all messages from data provider stream!
   */
  @Test
  ...
  /**
   * Join results from services `numberService1()` and `numberService2()` end-to-end. First
   * `numberService1` emits elements and then `numberService2`. (no interleaving)
   *
   * <p>Bonus: There are two ways to do this, check out both!
   */
  @Test
   ...
   *
   * <p>`taskExecutor()` returns tasks that should execute important work. Get all the tasks and
   * execute each of them.
   *
   * <p>Instead of flatMap() use concatMap() operator.
   *
   * <p>Answer: - What is difference between concatMap() and flatMap()? - What is difference between
   * concatMap() and flatMapSequential()? - Why doesn't Mono have concatMap() operator?
   */
  @Test
  ...
  /**
   * You are writing software for broker house. You can retrieve current stock prices by calling
   * either `getStocksGrpc()` or `getStocksRest()`. Since goal is the best response time, invoke
   * both services but use result only from the one that responds first.
   */
  @Test
  ...
  /**
   * As part of your job as software engineer for broker house, you have also introduced quick local
   * cache to retrieve stocks from. But cache may not be formed yet or is empty. If cache is empty,
   * switch to a live source: `getStocksRest()`.
   */
  @Test
  public void plan_b() {
  ...
  /**
   * You are checking mail in your mailboxes. Check first mailbox, and if first message contains
   * spam immediately switch to a second mailbox. Otherwise, read all messages from first mailbox.
   */
  @Test
  ...
  /**
   * You are implementing instant search for software company. When user types in a text box results
   * should appear in near real-time with each keystroke.
   *
   * <p>Call `autoComplete()` function for each user input but if newer input arrives, cancel
   * previous `autoComplete()` call and call it for latest input.
   */
  @Test
  ...
  /**
   * Code should work, but it should also be easy to read and understand. Orchestrate file writing
   * operations in a self-explanatory way using operators like `when`,`and`,`then`... If all
   * operations have been executed successfully return boolean value `true`.
   */
  @Test
  ...
  /**
   * News have come that Microsoft is buying Blizzard and there will be a major merger. Merge two
   * companies, so they may still produce titles in individual pace but as a single company.
   */
  @Test
  ...
  /**
   * Your job is to produce cars. To produce car you need chassis and engine that are produced by a
   * different manufacturer. You need both parts before you can produce a car. Also, engine factory
   * is located further away and engines are more complicated to produce, therefore it will be
   * slower part producer. After both parts arrive connect them to a car.
   */
  @Test
    ...
    StepVerifier.create(producedCars)
        ...
        .expectRecordedMatches(
            cars ->
                cars.stream()
                    .allMatch(
                        car -> Objects.equals(car.chassis.getSeqNum(), car.engine.getSeqNum())))
        .verifyComplete();
  }

  /**
   * When `chooseSource()` method is used, based on current value of sourceRef, decide which source
   * should be used.
   */
  ...
  /**
   * Sometimes you need to clean up after your self. Open a connection to a streaming service and
   * after all elements have been consumed, close connection (invoke closeConnection()), without
   * blocking.
   *
   * <p>This may look easy...
   */
  @Test
  public void cleanup() {
    BlockHound.install();
    ...
    Flux<String> stream =
        Flux.usingWhen(
            ...
            tr ->
                StreamingConnection
                    .closeConnection() // <-async complete, executes asynchronously after closure
            );
