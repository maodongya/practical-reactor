...
/**
 * With multi-core architectures being a commodity nowadays, being able to easily parallelize work
 * is important. Reactor helps with that by providing many mechanisms to execute work in parallel.
 *
 * <p>Read first:
 *
 * <p>https://projectreactor.io/docs/core/release/reference/#schedulers
...
public class c9_ExecutionControl extends ExecutionControlBase {
   ...
   * Since there could be multiple notifications, for better UX you want to slow down appearance
   * between notifications by 1 second. Pay attention to threading, compare what code prints out
   * before and after solution. Explain why?
   */
  @Test
  ...
  /**
   * You are using free access to remote hosting machine. You want to execute 3 tasks on this
   * machine, but machine will allow you to execute one task at a time on a given schedule which is
   * orchestrated by the semaphore. If you disrespect schedule, your access will be blocked. Delay
   * execution of tasks until semaphore signals you that you can execute the task.
   */
  @Test
  ...
  /**
   * Make task run on thread suited for short, non-blocking, parallelized work. Answer: - Which
   * types of schedulers Reactor provides? - What is their purpose? - What is their difference?
   */
  @Test
  ...
  /**
   * Make task run on thread suited for long, blocking, parallelized work. Answer: - What BlockHound
   * for?
   */
  @Test
  public void blocking() {
    ...
    Mono<Void> task =
        Mono.fromRunnable(ExecutionControlBase::blockingCall)
  ...
  public void free_runners() {
    ...
    Flux<Void> taskQueue = Flux.just(task, task, task).flatMap(Function.identity(), 3);

    // don't change code below
    Duration duration = StepVerifier.create(taskQueue).expectComplete().verify();

    Assertions.assertTrue(
        duration.getSeconds() <= 2, "Expected to complete in less than 2 seconds");
  }

  /**
   * Adapt the code so tasks are executed in parallel, but task results should preserve order in
   * which they are invoked.
   */
  @Test
  public void sequential_free_runners() {
    Flux<String> tasks =
        tasks().flatMapSequential(Function.identity()).doOnNext(System.out::println);

    // don't change code below
    Duration duration =
        StepVerifier.create(tasks).expectNext("1").expectNext("2").expectNext("3").verifyComplete();

    Assertions.assertTrue(
        duration.getSeconds() <= 1, "Expected to complete in less than 1 seconds");
  }

  /**
   * Make use of ParallelFlux to branch out processing of events in such way that: - filtering
   * events that have metadata, printing out metadata, and mapping to json can be done in parallel.
  ...
  public void event_processor() {
    ...
    List<String> steps =
        Scannable.from(eventStream).parents().map(Object::toString).collect(Collectors.toList());

    String last =
        Scannable.from(eventStream)
            .steps()
    ...
    Assertions.assertTrue(
        steps.contains("ParallelPeek"), "doOnNext operator not executed in parallel");
    Assertions.assertTrue(
        steps.contains("ParallelFilter"), "filter operator not executed in parallel");
