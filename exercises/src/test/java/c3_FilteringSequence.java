...
/**
 * Sequence may produce many elements, but we are not always interested in all of them. In this
 * chapter we will learn how to filter elements from a sequence.
 *
 * <p>Read first:
 *
 * <p>https://projectreactor.io/docs/core/release/reference/#which.filtering
 *
...
public class c3_FilteringSequence extends FilteringSequenceBase {
  ...
  public void girls_are_made_of_sugar_and_spice() {
    Flux<String> shortListed = popular_girl_names_service().filter(name -> name.length() <= 4);
  ...
  /**
   * `mashed_data_service()` returns sequence of generic objects. Without using `filter()` operator,
   * collect only objects that are instance of `String`
   */
  @Test
  ...
  public void economical() {
    ...
    StepVerifier.create(items).expectNext("1", "2", "3", "4", "5").verifyComplete();
  }

  /**
   * This service returns many elements, but you are only interested in the first one. Also, service
   * is very fragile, if you pull more than needed, you may brake it.
   *
   * <p>This time no blocking. Use only one operator.
   */
  @Test
  public void watch_out_for_the_spiders() {
    ...
    StepVerifier.create(firstResult).expectNext("watch_out").verifyComplete();
  }
  ...
  public void dont_take_more_then_you_need() {
    ...
    StepVerifier.create(numbers).expectNextCount(100).verifyComplete();
  }
