...
/**
 * Backpressure is a mechanism that allows a consumer to signal to a producer that it is ready
 * receive data. This is important because the producer may be sending data faster than the consumer
 * can process it, and can overwhelm consumer.
 *
 * <p>Read first:
 *
 * <p>https://projectreactor.io/docs/core/release/reference/#reactive.backpressure
...
public class c10_Backpressure extends BackpressureBase {
  ...
  /**
   * In this exercise subscriber (test) will request several messages from the message stream. Hook
   * to the requests and record them to the `requests` list.
   */
  @Test
  ...
  /**
   * Adjust previous solution in such a way that you limit rate of requests. Number of requested
   * messages stays the same, but each request should be limited to 1 message.
   */
  @Test
  ...
  /**
   * Finish the implementation of the `uuidGenerator` so it exactly requested amount of UUIDs. Or
   * better said, it should respect the backpressure of the consumer.
   */
  @Test
  public void uuid_generator() {
    Flux<UUID> uuidGenerator =
        Flux.create(
            sink ->
                sink.onRequest(
                    r -> {
  ...
  /**
   * You are receiving messages from malformed publisher that may not respect backpressure. In case
   * that publisher produces more messages than subscriber is able to consume, raise an error.
   */
  @Test
  ...
  /**
   * You are receiving messages from malformed publisher that may not respect backpressure. In case
   * that publisher produces more messages than subscriber is able to consume, buffer them for later
   * consumption without raising an error.
   */
  @Test
  ...
  /**
   * We saw how to react to request demand from producer side. In this part we are going to control
   * demand from consumer side by implementing BaseSubscriber directly. Finish implementation of
   * base subscriber (consumer of messages) with following objectives: - once there is subscription,
   * you should request exactly 10 messages from publisher - once you received 10 messages, you
   * should cancel any further requests from publisher. Producer respects backpressure.
   */
  @Test
